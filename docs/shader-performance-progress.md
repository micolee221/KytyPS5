...
The host Vulkan path retains monolithic graphics pipeline creation as its fallback. An optional, capability-gated graphics-pipeline-library path now creates and links a complete library. This validates the extension/device path but is not expected to remove first-use compilation cost until reusable partial libraries and background prewarming are implemented; the current slow-operation telemetry confirms that first-use `vkCreateGraphicsPipelines` calls can block the render thread for hundreds of milliseconds.
